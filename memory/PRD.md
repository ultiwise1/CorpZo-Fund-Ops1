...
## Iteration 9 (Feb 2026) — Partner CRM v2 + CAM Full-Form polish
- **Partner CRM v2** (`/channel-partners`, rebuilt `ChannelPartners.jsx` + new backend endpoints in `server.py`):
  - New endpoint `GET /api/partners/performance` — returns `{kpis:{active_partners,total_partners,mtd_disbursement,mtd_commission_accrued,overdue_payouts}, period, rows:[…]}`. Each row carries `partner_uid, channel_code, name, city, status, products, monthly_target, leads, cases, disbursed_total, disbursed_mtd, commission_total, commission_mtd, commission_payable, commission_overdue, attainment_pct, trend_labels[6], trend_disbursed[6]`. Rows sorted by MTD disbursement DESC.
  - New endpoint `PATCH /api/channel-partners/{uid}` — allow-listed field updates (name, entity_name, pan, gst, status, kyc_status, products, geography, channel_manager_uid, commission_structure, mobile, email, city, state, bank_account, **monthly_target**, notes). Roles: super_admin, business_head, channel_manager, finance.
  - New UI: 4 KPI cards (Active partners · MTD disb · MTD commission · Overdue payouts), Top-5 leaderboard by MTD disbursement, All-partners table with editable monthly target and inline **attainment bar** (green ≥90 · amber ≥60 · red <60), search across name/code/city/uid, and a Partner Detail Dialog with 3 tiles, a 6-month **SVG sparkline**, target editor + save, and a contact rail with commission-payable & overdue-payout metrics.
- **CAM Full-Form polish** (`CAM.jsx`):
  - **Auto-flag suggestions** derived from ratios + banking + bureau + doc %: DSCR (<1.2 red, <1.5 amber, ≥1.5 green), FOIR (>60 red, >40 amber), D/E (>3 red), Current ratio (<1 red), Debt/EBITDA (>4.5 red), Bureau score (<650 red, <700 amber, ≥750 green), Doc % (<50 red, <70 amber), Cheque returns >3 red, Cash deposits >30% amber. One-click **Accept →** per suggestion or **Accept all** to consume the whole list.
  - **Ratio input color hints** — Current, D/E, TOL/TNW, Debt/EBITDA, DSCR, FOIR fields now render the input border in green/amber/red based on Indian credit-market thresholds, with a small hint caption under each field.
  - **Print CAM to PDF** — new `GET /api/cases/{case_uid}/cam.pdf` uses `reportlab` to produce a branded A4 memo (snapshot table, business overview, financial performance, banking, ratios, positives+concerns, colour-coded flags table, indicative eligibility + recommendation + analyst comments). Downloadable via [data-testid=cam-pdf-btn] on the CAM tab.
- **Server-side hardening**:
  - `POST /api/cases/{uid}/assessment` now validates the `flags` array — each entry must be `{level: green|amber|red, title: str}` and auto-fills `id` if missing; string / malformed flags return 422.
